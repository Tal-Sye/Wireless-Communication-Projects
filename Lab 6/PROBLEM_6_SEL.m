...
for k = 1:length(SNR)
    ...
    for m = 1:TRIAL
        ...
        if(abs(h1)>abs(h2) && abs(h1)>abs(h3) && abs(h1)>abs(h4))
        b_dec=sign(r1);
        error_q= error_q+qfunc(sqrt((abs(h1)^2*2*P(k)/N0))); 
        end

        if(abs(h2)>abs(h1) && abs(h2)>abs(h3) && abs(h2)>abs(h4))
        b_dec=sign(r2);
        error_q= error_q+qfunc(sqrt((abs(h2)^2*2*P(k)/N0))); 
        end 

        if(abs(h3)>abs(h2) && abs(h3)>abs(h1) && abs(h3)>abs(h4))
        b_dec=sign(r3);
        error_q= error_q+qfunc(sqrt((abs(h3)^2*2*P(k)/N0))); 
        end 

        if(abs(h4)>abs(h2) && abs(h4)>abs(h3) && abs(h4)>abs(h1))
        b_dec=sign(r4);
        error_q= error_q+qfunc(sqrt((abs(h4)^2*2*P(k)/N0))); 
        end
